import { readFileSync } from 'node:fs';
import { defineConfig } from 'wxt';

const { version } = JSON.parse(
  readFileSync(new URL('../../package.json', import.meta.url), 'utf-8'),
);

export default defineConfig({
  srcDir: 'src',
  entrypointsDir: 'entrypoints',
  modules: ['@wxt-dev/module-react'],
  hooks: {
    'entrypoints:found'(wxt, entrypoints) {
      const demoIndex = entrypoints.findIndex(({ name }) => name === 'demo');
      if (wxt.config.command === 'build' && wxt.config.mode !== 'demo' && demoIndex >= 0) {
        entrypoints.splice(demoIndex, 1);
      }
    },
  },
  manifest: {
    name: 'OrderCue',
    key: '',
    version,
    description: 'Manage and track your Amazon orders',
    permissions: ['storage', 'activeTab', 'sidePanel', 'identity', 'alarms', 'notifications'],
    host_permissions: [
      '*://*.amazon.com/*',
      '*://*.facebook.com/*',
      'https://order-wizard-api.fly.dev/*',
      'https://*.amazoncognito.com/*',
      'https://cognito-idp.us-west-1.amazonaws.com/*',
    ],
    action: {
      default_title: 'Open OrderCue',
      default_icon: {
        16: 'icon-16.png',
        32: 'icon-32.png',
        48: 'icon-48.png',
        128: 'icon-128.png',
      },
    },
    icons: {
      16: 'icon-16.png',
      32: 'icon-32.png',
      48: 'icon-48.png',
      128: 'icon-128.png',
    },
    side_panel: {
      default_path: 'sidepanel.html',
    },
    web_accessible_resources: [
      {
        resources: ['listing-preview.html'],
        matches: ['*://*.amazon.com/*', '*://*.facebook.com/*'],
      },
    ],
  },
  vite: () => ({
    resolve: {
      alias: {
        '@': '/src',
      },
    },
  }),
});

import { Check, ExternalLink, StickyNote, Trash2, TriangleAlert, X } from 'lucide-react';
...
interface OrderCardProps {
  ...
  returnWarning?: ReturnWarning | undefined;
  isSelected: boolean;
...
function OrderCardImpl({
  ...
  const [edit, setEdit] = useState({ orderId: order.id, baseline: savedNote, value: savedNote });
  const draftNote = edit.value;
  const setDraftNote = (value: string) => setEdit((previous) => ({ ...previous, value }));
  const isDirty = draftNote !== savedNote;
  const orderUrl = getAmazonOrderUrl(order.orderNumber);

  useEffect(() => {
    setEdit((previous) => {
      if (previous.orderId === order.id && previous.baseline === savedNote) {
        return previous;
      }
      const value =
        previous.orderId !== order.id || previous.value === previous.baseline
          ? savedNote
          : previous.value;
      return { orderId: order.id, baseline: savedNote, value };
    });
  }, [order.id, savedNote]);
